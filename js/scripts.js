...
window.addEventListener('DOMContentLoaded', event => {
    ...
    // ===== Chat widget -> backend bridge (non-blocking cold-start handling) =====
    if (!window.__chatInited) {
      ...
      // Warmup in background (non-blocking; ignore errors)
      (async function warmupInBackground(){
        try {
          const healthURL = BACKEND_URL.replace(/\/chat$/, '/health');
          try { await fetchWithTimeout(healthURL, {}, 5000); } catch {}
          try {
      ...
      // Ask with retries, but never block UI waiting on warmup
      async function askAboutKshitij(message) {
        // 3 attempts total: immediate, then after 1.2s, then after 3s
        ...
        throw lastErr || new Error("Cold start failed");
      }
      ...
      async function sendMsg() {
        ...
        try {
          ...
          hideTyping();
          append("Kshitij", "Server is waking up. Please try again in a moment 🙏");
        } finally {
